...
app.get('/', (req, res) => {
    ...
    `);
});>SKTorrent RealDebrid Addon</title>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width, initial-scale=1">
            <style>
                body {
        ...
        <body>
            <div class="container">
                <h1>⚡ SKTorrent RealDebrid</h1>
                <p class="subtitle">Bezpečný addon pro přehrávání SKTorrent obsahu přes Real-Debrid</p>

                <div class="setup-section">
                    <h2>🔧 Nastavení Real-Debrid API</h2>
                    <p>Pro použití addonu potřebujete <strong>Real-Debrid API klíč</strong>:</p>
                    
                    <div class="instructions">
                        <h3>📋 Jak získat API klíč:</h3>
                        <ol>
                            <li>Přihlaste se na <a href="https://real-debrid.com" target="_blank">Real-Debrid.com</a></li>
                            <li>Přejděte na <strong>Account → API</strong></li>
                            <li>Klikněte na <strong>Generate</strong></li>
                    ...
                    <form id="setupForm">
                        ...
                        <button type="submit" class="btn">💾 Uložit a vygenerovat instalační URL</button>
                    </form>

                    <div id="success" class="success">
                        <h3>✅ Úspěšně nakonfigurováno!</h3>
                        <p>Váš addon je připraven k instalaci.</p>
                        <div id="installUrl" class="install-url"></div>
                        <button type="button" class="copy-btn" onclick="copyToClipboard()">📋 Kopírovat URL</button>
                        <br><br>
                        <a href="#" id="stremioLink" class="btn">🚀 Instalovat do Stremio</a>
                ...
                <div style="text-align: center; margin-top: 40px; color: #718096;">
                    <p><strong>Powered by:</strong> Real-Debrid API + SKTorrent.eu + Render.com</p>
                </div>
            </div>
...
app.use((req, res, next) => {
    // Extrahovat user ID z URL pro stream requesty
    const urlMatch = req.url.match(/\/([a-f0-9]{32})\.json/);
    if (urlMatch) {
        req.headers['x-user-id'] = urlMatch[1];
    }
    
    // Pro stream requesty, získat user ID z referer
    if (req.url.startsWith('/stream/')) {
        const referer = req.get('Referer') || req.get('Origin') || '';
        const userIdMatch = referer.match(/\/([a-f0-9]{32})\.json/);
        if (userIdMatch) {
            req.headers['x-user-id'] = userIdMatch[1];
        }
    }
...
// Mount addon router s custom middleware
app.use('/', addonRouter);
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🔧 SKTorrent účet: ${SKT_UID ? 'Nakonfigurován' : 'NENÍ NAKONFIGUROVÁN'}`);
    console.log(`💾 Cache: In-memory storage`);
    console.log(`📊 Environment: ${process.env.NODE_ENV || 'development'}`);
    
    if (!SKT_UID || !SKT_PASS) {
        console.warn('⚠️ VAROVÁNÍ: SKT_UID nebo SKT_PASS nejsou nastaveny!');
    }
});